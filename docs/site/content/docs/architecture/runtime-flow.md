...
1. Shared prompt-composer helpers in `crates/agentty/src/domain/composer.rs`
   derive slash-menu options, attachment-aware deletion ranges, and the drained
   prompt submission payload before runtime hands the turn to the app layer.
   The transcript keeps raw `@path` lookups, while the later agent-facing
   prompt text quotes the repository-relative path without adding transport
   sentinels.
